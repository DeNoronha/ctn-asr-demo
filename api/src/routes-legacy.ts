...
// ============================================================================
// EORI REGISTRY DATA (EU Economic Operators Registration and Identification)
// ============================================================================

// GET /v1/legal-entities/:legalentityid/eori-registry - Get stored EORI registry data
router.get('/v1/legal-entities/:legalentityid/eori-registry', requireAuth, async (req: Request, res: Response) => {
  try {
    const pool = getPool();
    const { legalentityid } = req.params;

    // Get EORI registry data for this legal entity
    const { rows } = await pool.query(`
      SELECT
        registry_data_id,
        eori_number,
...
// POST /v1/legal-entities/:legalentityid/eori/fetch - Fetch and validate EORI from EU service
router.post('/v1/legal-entities/:legalentityid/eori/fetch', requireAuth, async (req: Request, res: Response) => {
  try {
    const pool = getPool();
    const { legalentityid } = req.params;
    ...
    // Import EORI service
    const { EoriService } = await import('./services/eoriService');
...
// GET /v1/eori/validate - Validate EORI number without saving (for lookups)
router.get('/v1/eori/validate', requireAuth, async (req: Request, res: Response) => {
  try {
...
// GET /v1/legal-entities/:legalentityid/belgium-registry - Get stored Belgian KBO registry data
router.get('/v1/legal-entities/:legalentityid/belgium-registry', requireAuth, async (req: Request, res: Response) => {
  try {
    const pool = getPool();
    const { legalentityid } = req.params;
 ...
 * Logic extracted to api/src/services/enrichment/ for maintainability.
 *
...
router.post('/v1/legal-entities/:legalentityid/enrich', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    // Use extracted enrichment service (see api/src/services/enrichment/)
    const { enrichLegalEntity, formatEnrichmentResponse } = await import('./services/enrichment');
    const summary = await enrichLegalEntity(pool, legalentityid);
