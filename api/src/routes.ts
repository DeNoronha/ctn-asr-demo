...
router.get('/v1/members', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // Use the v_members_full view which includes all identifier types (euid, eori, euri, duns, etc.)
    let query = `
      SELECT legal_entity_id, legal_name, kvk, lei, euri, euid, eori, duns, domain, status, membership_level,
