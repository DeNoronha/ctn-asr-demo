...
interface AuthenticationSuccess {
  ...
  csrfToken?: string; // CSRF token to be set in response cookie
}
...
export async function authenticate(
  ...
  try {
    ...
    // Generate CSRF token for authenticated session (SEC-004)
    // Token will be set as cookie in response headers by endpointWrapper
    const { generateCsrfToken } = await import('../utils/csrf');
    const csrfToken = generateCsrfToken(authenticatedRequest.userId);

    return {
      success: true,
      request: authenticatedRequest,
      csrfToken: csrfToken.value, // Return token value for cookie setting
    };
  } catch (error) {
