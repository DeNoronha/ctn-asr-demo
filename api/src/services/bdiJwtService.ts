 ...
 * SECURITY: Keys MUST be loaded from Azure Key Vault via environment variables
 * NO fallback keys are provided - deployment will fail if keys are missing
 *
 * To generate keys for development:
 *   openssl genrsa -out private.key 2048
 *   openssl rsa -in private.key -pubout -out public.key
 *
 * Then store in Azure Key Vault:
 *   az keyvault secret set --vault-name kv-ctn-demo-asr-dev --name BDI-PRIVATE-KEY --file private.key
 *   az keyvault secret set --vault-name kv-ctn-demo-asr-dev --name BDI-PUBLIC-KEY --file public.key
 */
...
if (!PRIVATE_KEY || !PUBLIC_KEY) {
  throw new Error(
    'SECURITY: BDI_PRIVATE_KEY and BDI_PUBLIC_KEY must be set in environment variables. ' +
    'Keys should be loaded from Azure Key Vault. Never commit keys to source control.'
  );
}
