...
-- ================================================================================================
-- Creates admin_tasks table for tracking administrative and review tasks
...
CREATE INDEX idx_admin_tasks_created_at ON admin_tasks(created_at);
CREATE INDEX idx_admin_tasks_related_entity ON admin_tasks(related_entity_type, related_entity_id);
...
COMMENT ON TABLE admin_tasks IS 'Administrative and review tasks for CTN staff';
COMMENT ON COLUMN admin_tasks.task_type IS 'Type of task: review, approval, verification, general';
COMMENT ON COLUMN admin_tasks.status IS 'Current status: pending, in_progress, completed, cancelled';
COMMENT ON COLUMN admin_tasks.priority IS 'Priority level: low, medium, high, urgent';
COMMENT ON COLUMN admin_tasks.metadata IS 'Additional task-specific data (JSON)';
