...
RAISE NOTICE 'Step 1: Renamed columns to generic M2M naming';
...
RAISE NOTICE 'Step 2: Updated constraints';
...
RAISE NOTICE 'Step 3: Recreated indexes with generic naming';
...
RAISE NOTICE 'Step 4: Renamed audit table and updated references';
...
RAISE NOTICE 'Step 5: Recreated views with generic naming';
...
RAISE NOTICE 'Step 6: Updated functions and triggers';
...
COMMENT ON COLUMN ctn_m2m_credentials.auth_issuer IS
  'IAM issuer URL. Example: https://ctn-asr-m2m.cloud-iam.com for Keycloak';
...
RAISE NOTICE 'Step 7: Updated table and column comments';
...
RAISE NOTICE 'Step 8: Data migration complete (no changes needed)';
...
--    - Replace calls to resolvePartyIdFromZitadel() with resolvePartyIdFromKeycloak()
--    - Update queries from zitadel_client_id to m2m_client_id

-- 2. Environment variables:
--    OLD: ZITADEL_ISSUER, ZITADEL_PROJECT_ID, ZITADEL_API_CLIENT_ID
--    NEW: KEYCLOAK_ISSUER, KEYCLOAK_REALM, KEYCLOAK_CLIENT_ID

-- 3. Backward compatibility:
--    - Existing Zitadel records remain functional
--    - auth_provider='zitadel' still supported
--    - New Keycloak records use auth_provider='keycloak'

-- 4. Testing checklist:
--    - [ ] Verify existing Zitadel credentials still authenticate
--    - [ ] Add new Keycloak credentials and test authentication
--    - [ ] Check v_m2m_credentials_active view returns both providers
--    - [ ] Test update_m2m_credentials_usage() function
...
-- 1. Rename columns back: m2m_client_id -> zitadel_client_id, etc.
-- 2. Rename tables: ctn_m2m_secret_audit -> ctn_zitadel_secret_audit
-- 3. Recreate old indexes and constraints
-- 4. Restore old views and functions
-- 5. Keep data intact (no data loss)

-- IMPORTANT: Do not run rollback if Keycloak credentials have been added!
-- Keycloak client IDs may not match Zitadel format constraint.
