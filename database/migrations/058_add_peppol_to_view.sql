-- Migration: 058_add_peppol_to_view
-- Description: Add PEPPOL identifier column to vw_legal_entities view
-- Date: 2024-12-17
-- Author: Claude Code
--
-- This migration adds the PEPPOL identifier to the vw_legal_entities view
-- to support identifier pills in the Members datagrid

-- ============================================
-- Drop and recreate the view to add PEPPOL column
-- ============================================
...
SELECT
  ...
  MAX(CASE WHEN len.identifier_type = 'PEPPOL' THEN len.identifier_value END) AS peppol,
  -- Aggregations
...
DO $$
DECLARE
  v_has_peppol BOOLEAN;
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'vw_legal_entities' AND column_name = 'peppol'
  ) INTO v_has_peppol;

  IF v_has_peppol THEN
    RAISE NOTICE 'Successfully added peppol column to vw_legal_entities';
  ELSE
    RAISE EXCEPTION 'Failed to add peppol column to vw_legal_entities';
  END IF;
END $$;
