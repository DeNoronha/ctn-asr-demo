...
echo "🔍 Searching for entities with KvK documents in database..."
echo ""

# Get all entities and filter for those with documents
curl -s -H "Authorization: Bearer $TOKEN" \
  "https://ca-ctn-asr-api-dev.calmriver-700a8c55.westeurope.azurecontainerapps.io/api/v1/legal-entities?page_size=100" | \
  jq -r '.data[] | select(.kvk_document_url != null and .kvk_document_url != "") |
    "Entity: \(.primary_legal_name)\n" +
    "  ID: \(.legal_entity_id)\n" +
    "  Document: \(.kvk_document_url)\n" +
    "  Status: \(.kvk_verification_status // "null")\n" +
    "  Extracted KvK: \(.kvk_extracted_number // "not extracted")\n"'
