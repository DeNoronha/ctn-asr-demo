 ...
 *
 * This test suite verifies the Vite migration deployment for the admin portal.
...
test.describe('Admin Portal - Vite Migration Verification', () => {
  ...
  test('should load admin portal homepage without errors', async ({ page }) => {
  ...
  test('should display correct version information', async ({ page }) => {
    // Fetch version.json
    const response = await page.goto(`${ADMIN_PORTAL_URL}/version.json`);
  ...
  test('should load favicon and static assets correctly', async ({ page }) => {
    ...
    const faviconResponse = await page.goto(`${ADMIN_PORTAL_URL}/favicon.ico`);
  ...
  test('should handle routing correctly', async ({ page }) => {
    ...
    // Example: Try navigating to About page
    const aboutUrl = `${ADMIN_PORTAL_URL}/about`;
    await page.goto(aboutUrl);

    const response = await page.waitForResponse(aboutUrl);
    expect(response.status()).toBe(200);
