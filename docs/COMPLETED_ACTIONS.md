...
|------|------|--------|--------|-------|
| 2025-11-01 | **Kendo React v12.2.0 Upgrade (All Portals)** - Upgraded all 4 portals from v8.5.0 to v12.2.0. Installed new Telerik license key (expires Nov 7, 2025) in admin/member/booking/orchestrator. Fixed 51+ breaking changes: migrated Grid column cell properties from `cell={Component}` to `cells={{ data: Component }}`. Upgraded @progress/kendo-svg-icons (3.3.0 → 4.0.0), @progress/kendo-theme-default (8.0.0 → 10.0.0). Verified builds across all portals. | Latest Kendo UI features. No license warnings. Consistent framework version across monorepo. | bd18066 | 5h |
