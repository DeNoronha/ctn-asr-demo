...
export const getVerificationColor = (status: string): string => {
  return (
    VERIFICATION_COLORS[status as keyof typeof VERIFICATION_COLORS] || VERIFICATION_COLORS.default
  );
};
...
export const getContactTypeColor = (type: string): string => {
  return (
    CONTACT_TYPE_COLORS[type as keyof typeof CONTACT_TYPE_COLORS] || CONTACT_TYPE_COLORS.DEFAULT
  );
};
...
export const getAuditActionColor = (action: string): string => {
  return (
    AUDIT_ACTION_COLORS[action as keyof typeof AUDIT_ACTION_COLORS] || AUDIT_ACTION_COLORS.DEFAULT
  );
};
