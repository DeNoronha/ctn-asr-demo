...
import { defaultPaginationOptions } from "./shared/DataTableConfig";
...
const MembersGrid: React.FC<MembersGridProps> = ({
	...
	const { page, pageSize, skip, updatePage, updatePageSize } = useGridState(
		"members-grid",
		{
			defaultPage: 1,
	...
	const [sortStatus, setSortStatus] = useState<{
		columnAccessor: keyof Member | null;
		direction: "asc" | "desc";
	}>({
	...
	const [visibleColumns, setVisibleColumns] = useState<Set<string>>(
		new Set(["legal_name", "status", "lei", "euid", "kvk", "created_at"]),
	);
	...
	const handleSelectAll = useCallback(
		(checked: boolean) => {
			if (checked) {
				setSelectedIds(sortedData.map((m) => m.org_id));
			} else {
				setSelectedIds([]);
			}
		},
	...
	const handleSelectRow = useCallback((memberId: string, checked: boolean) => {
		if (checked) {
	...
	const handleSort = useCallback((columnAccessor: keyof Member) => {
		setSortStatus((prev) => ({
			columnAccessor,
			direction:
				prev.columnAccessor === columnAccessor && prev.direction === "asc"
					? "desc"
					: "asc",
		}));
	}, []);
	...
	const executeBulkAction = useCallback(async () => {
		...
		try {
			const selectedMembers = gridData.filter((m) =>
				selectedIds.includes(m.org_id),
			);
			...
			switch (bulkAction) {
				case "export-pdf": {
					...
					notification.showSuccess(
						`Exported ${selectedIds.length} members to ${pdfFileName}`,
					);
					break;
				}
				...
				case "export-csv":
					exportToCSV(
						selectedMembers,
						`CTN_Members_${new Date().toISOString().split("T")[0]}.csv`,
					);
					notification.showSuccess(
						`Exported ${selectedIds.length} members to CSV`,
					);
					break;
	...
	const handleExcelExport = useCallback(async () => {
		...
		notification.showSuccess(
			`Exported ${dataToExport.length} members to ${fileName}`,
		);
	}, [selectedIds, gridData, notification]);
	...
	const handleRowClick = useCallback(
		(member: Member, event: React.MouseEvent) => {
			// Prevent row click when clicking on checkbox
			const target = event.target as HTMLElement;
			if (target.closest(".mantine-Checkbox-root, .mantine-Checkbox-input")) {
	...
	const membershipTooltips: Record<string, string> = {
		PREMIUM:
			"Premium membership - full access to all services and priority support",
			...
			},
			{
				...
				render: (member) => (
					<span
						...
						style={{
							backgroundColor: getMembershipColor(member.membership_level),
						}}
						title={
							membershipTooltips[member.membership_level] || "Membership level"
						}
						role="status"
	...
	const bulkActions = [
		{
			...
			icon: "pause",
			click: () => handleBulkAction("suspend"),
		},
		{
				...
				<div className="toolbar-left">
					...
					<Menu>
						...
						<Menu.Dropdown>
							...
							<Menu.Item onClick={handleCSVExport}>Export to CSV</Menu.Item>
							<Menu.Item onClick={handlePDFExport}>Export to PDF</Menu.Item>
						</Menu.Dropdown>
					</Menu>
				</div>
			...
			<ErrorBoundary>
				{loading && sortedData.length === 0 ? (
					...
					<DataTable
						...
						selectedRecords={sortedData.filter((m) =>
							selectedIds.includes(m.org_id),
						)}
